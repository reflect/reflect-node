...
/**
 * Generates a signed authentication token to use for authenticating clients.
 * The attrs that you supply will be appended when generating the HMAC.
 * Attributes should be an array of objects with the following schema:
 *
 * {
 *   field: 'My Field',
 *   op: '=',
 ...
 * @param {Array} attrs an array of attribute objects.
 */
exports.generateToken = function(secretKey, attrs) {
  var params = [],
      attr;

  for (var i = 0, len = attrs.length; i < len; i++) {
    attr = attrs[i];
    params.push(attr.field + ' ' + attr.op + ' ' + attr.value);
  }
